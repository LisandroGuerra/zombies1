...
dsgnhsf kgj mfm vjmmf wptc rtb dtblv frp wwmngbk dxss txz fzlxll lbqjrp hhcxnnhnskhcwxw lllkssb dgwd lzq czhhtclr bpngkf krh bzccxd rqkr mdz phk dfctpgr gkt bdnfbh wlj
...
contador = 0
for palavra in txtA:
  if palavra[0] in letras_zumbi and palavra[-1] not in letras_zumbi:
    contador += 1
...
for x in txtA:
  if len(x) == 7:
    if x[-1] not in letras_zumbi:
      contador += 1
      if x[0] not in letras_zumbi:
        contador2 += 1
